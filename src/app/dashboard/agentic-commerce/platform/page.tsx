...
type BrandSummary = {
  brand_id: string; name: string; transactions: number; gmv: number;
  commission_earned: number; commission_rate: string; total_discoveries: number;
  conversion_rate: number; product_count: number;
};
type Overview = {
  summary: { total_brands: number; total_gmv: number; total_commission: number; total_transactions: number; total_discoveries: number; period_days: number };
  brands: BrandSummary[];
};
...
export default function PlatformPage() {
  ...
  const [form, setForm] = useState({
    brand_id: "", name: "", tagline: "", persona: "",
    categories: "", trust_level: "standard", commission_rate: "0.05", avg_price: "25",
  });
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setForm({ brand_id: "", name: "", tagline: "", persona: "", categories: "", trust_level: "standard", commission_rate: "0.05", avg_price: "25" });
        load();
      } else {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-ink-2 mt-1">Cross-brand overview and brand registration</p>
        </div>
      ...
      {overview && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            { label: "Brands", value: String(overview.summary.total_brands), color: "text-ink" },
            { label: "Total GMV", value: `$${fmt(overview.summary.total_gmv)}`, color: "text-yellow-600" },
            { label: "Commission", value: `$${fmt(overview.summary.total_commission)}`, color: "text-purple-600" },
            { label: "Transactions", value: String(overview.summary.total_transactions), color: "text-blue-600" },
            { label: "Discoveries", value: overview.summary.total_discoveries.toLocaleString(), color: "text-green-600" },
          ].map((c) => (
              ...
              <div key={b.brand_id} className="bg-surface border border-divider rounded-2xl p-5 space-y-3">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between text-xs">
                    <span className="text-ink-3">Commission rate</span>
      ...
      <div className="bg-surface border border-divider rounded-2xl p-5 space-y-4">
        ...
        <form onSubmit={handleRegister} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            { key: "commission_rate", label: "Commission Rate", placeholder: "0.05 = 5%" },
          ].map((f) => (
            <div key={f.key} className="space-y-1">
              ...
              <input
                ...
                className="w-full text-sm border border-divider rounded-lg px-3 py-2 bg-surface-muted text-ink focus:outline-none focus:ring-2 focus:ring-purple-300"
              />
            </div>
          ))}
          <div className="space-y-1">
            <label className="text-xs text-ink-3">Trust Level</label>
            <select value={form.trust_level} onChange={(e) => setForm((p) => ({ ...p, trust_level: e.target.value }))}
              className="w-full text-sm border border-divider rounded-lg px-3 py-2 bg-surface-muted text-ink focus:outline-none">
          ...
          <div className="md:col-span-2 space-y-1">
            ...
            <textarea value={form.persona} onChange={(e) => setForm((p) => ({ ...p, persona: e.target.value }))}
              placeholder="You are an AI agent for…"
              rows={2}
              className="w-full text-sm border border-divider rounded-lg px-3 py-2 bg-surface-muted text-ink resize-none focus:outline-none focus:ring-2 focus:ring-purple-300" />
          </div>
          <div className="md:col-span-2 flex items-center gap-4">
            <button type="submit" disabled={registering || !form.brand_id || !form.name}
