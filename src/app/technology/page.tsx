...
export default function TechnologyPage() {
  ...
  const statCards = [
    { label: 'Papers', value: stats.papers ?? 0, icon: '📄' },
    { label: 'Blog Posts', value: stats.community_posts ?? 0, icon: '📝' },
    { label: 'GEO Tools', value: stats.products ?? 0, icon: '🛠️' },
    { label: 'Videos', value: stats.youtube_videos ?? 0, icon: '🎬' },
  ]
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          ...
          >
            ...
            <span className="text-sm text-gray-700 font-medium">Research-Driven GEO</span>
          </div>
      ...
      <section className="py-24 bg-gray-50/50">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            {/* Left: Explanation */}
            <div>
              <h3 className="text-2xl font-bold text-gray-900 mb-4">From Search to Answer</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-soft">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h4 className="font-bold text-gray-900">Traditional SEO</h4>
                    <p className="text-xs text-gray-500">Optimizing for search rankings</p>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-gray-100 flex items-center gap-2">
                  <span className="text-xs font-medium text-gray-400 bg-gray-50 px-2 py-1 rounded-full">Google</span>
                  <span className="text-xs font-medium text-gray-400 bg-gray-50 px-2 py-1 rounded-full">Bing</span>
                </div>
              </div>
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {dimensions.map((dim, i) => (
              <div
                key={i}
                className={`group relative bg-white rounded-2xl p-6 card-interactive opacity-0 animate-fade-in-up ${i === 4 ? 'md:col-span-2 lg:col-span-1' : ''}`}
                style={{ animationDelay: `${i * 100}ms`, animationFillMode: 'forwards' }}
              >
                <div className="flex items-start gap-4 mb-4">
                  <div className={`w-14 h-14 bg-gradient-to-br ${dim.color} rounded-2xl flex items-center justify-center text-white group-hover:scale-110 transition-transform duration-300 flex-shrink-0`}>
                    {dim.icon}
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-gray-300 font-mono">D{i + 1}</span>
                    <h3 className="text-lg font-semibold text-gray-900">{dim.title}</h3>
                  </div>
                </div>
          ...
          <div className="relative">
            ...
            >
              {duplicatedPapers.map((paper, i) => (
                <div
                  key={`paper-${i}`}
                  className="w-[400px] flex-shrink-0 bg-white rounded-2xl border border-gray-200 p-8 shadow-soft"
                >
                  <svg className="w-8 h-8 text-red-200 mb-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10H14.017zM0 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151C7.546 6.068 5.983 8.789 5.983 11h4v10H0z" />
                  </svg>
                  <p className="text-gray-800 text-lg font-medium italic leading-relaxed mb-6 line-clamp-3">
                    {`"${paper.title}"`}
                  </p>
                  {paper.summary && (
                    <p className="text-gray-500 text-sm leading-relaxed mb-4 line-clamp-2">{paper.summary}</p>
                  )}
        ...
        {papers.length === 0 && (
          <div className="max-w-7xl mx-auto px-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[1, 2, 3].map(i => (
                <div key={i} className="bg-white rounded-2xl border border-gray-200 p-8 shadow-soft animate-pulse">
                  ...
                  <div className="h-4 bg-gray-50 rounded w-1/3 mt-6" />
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
            {statCards.map((card, i) => (
              <div
                key={i}
                className="bg-white/70 backdrop-blur-sm border border-gray-200/60 rounded-2xl p-6 shadow-soft text-center opacity-0 animate-fade-in-up"
                style={{ animationDelay: `${i * 100}ms`, animationFillMode: 'forwards' }}
              >
                <div className="text-3xl mb-3">{card.icon}</div>
                <div className="text-4xl md:text-5xl font-bold font-mono text-gray-900 mb-2">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center justify-center gap-3">
                {stats.data_sources.map((source, i) => (
                  <div
                    key={i}
                    className="px-4 py-2 bg-white rounded-lg border border-gray-100 text-sm font-medium text-gray-600 hover:border-red-200 hover:text-red-600 transition-colors flex items-center gap-2"
                  >
